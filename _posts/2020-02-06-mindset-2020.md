...
现在找到了一种提高体验的方式，安装VSCode的插件**Paste Image**,直接利用截图工具截图到剪切板，在VSCode里CTRL+SHIFT+P执行命令 Paste Image
就可以自动地像上面一样往Markdown文件里插入图片， 也可以直接通过快捷键 CTRL+ALT+V 插入剪切板的图片。 当然安装好插件**Paste Image**后，在设置里也可以根据个人实际情况做一些定制。
